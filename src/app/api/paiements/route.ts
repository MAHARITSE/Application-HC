...
export async function GET(request: Request) {
  try {
    ...
    if (conditions.length > 0) {
      result = await db.select().from(paiements)
...
export async function POST(request: Request) {
  try {
    ...
    const [result] = await db.insert(paiements).values({
      enseignantId: Number(body.enseignantId),
      anneeId: Number(body.anneeId),
      montantAvance: body.montantAvance || 0,
      dateAvance: body.dateAvance || null,
      pourcentageTranche: body.pourcentageTranche || 100,
